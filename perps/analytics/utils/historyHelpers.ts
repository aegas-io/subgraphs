...
export function updateHistories(params: UpdateHistoriesParams): void {
	...
	dh.openFee = (dh.openFee ? dh.openFee! : BigInt.zero()).plus(params._openFee)
	dh.closeFee = (dh.closeFee ? dh.closeFee! : BigInt.zero()).plus(params._closeFee)
	...
	if (params.solver) {
		...
		sdh.openFee = (sdh.openFee ? sdh.openFee! : BigInt.zero()).plus(params._openFee)
		sdh.closeFee = (sdh.closeFee ? sdh.closeFee! : BigInt.zero()).plus(params._closeFee)
		if (params._positionsCount.gt(BigInt.zero())) {
		...
		sodh.openFee = (sodh.openFee ? sodh.openFee! : BigInt.zero()).plus(params._openFee)
		sodh.closeFee = (sodh.closeFee ? sodh.closeFee! : BigInt.zero()).plus(params._closeFee)
		if (params._positionsCount.gt(BigInt.zero())) {
	...
	th.openFee = (th.openFee ? th.openFee! : BigInt.zero()).plus(params._openFee)
	th.closeFee = (th.closeFee ? th.closeFee! : BigInt.zero()).plus(params._closeFee)
	...
	duh.openFeePaid = (duh.openFeePaid ? duh.openFeePaid! : BigInt.zero()).plus(params._openFee)
	duh.closeFeePaid = (duh.closeFeePaid ? duh.closeFeePaid! : BigInt.zero()).plus(params._closeFee)
	...
	tuh.openFeePaid = (tuh.openFeePaid ? tuh.openFeePaid! : BigInt.zero()).plus(params._openFee)
	tuh.closeFeePaid = (tuh.closeFeePaid ? tuh.closeFeePaid! : BigInt.zero()).plus(params._closeFee)
	...
	if (params._symbolId.gt(BigInt.zero())) {
		...
		dst.volume = (dst.volume ? dst.volume! : BigInt.zero()).plus(tradeVolume)
		dst.totalTrades = dst.totalTrades.plus(BigInt.fromString("1"))
		dst.platformFeePaid = dst.platformFeePaid.plus(totalFee)
		dst.openFeePaid = (dst.openFeePaid ? dst.openFeePaid! : BigInt.zero()).plus(params._openFee)
		dst.closeFeePaid = (dst.closeFeePaid ? dst.closeFeePaid! : BigInt.zero()).plus(params._closeFee)
		...
		tst.volume = (tst.volume ? tst.volume! : BigInt.zero()).plus(tradeVolume)
		tst.totalTrades = tst.totalTrades.plus(BigInt.fromString("1"))
		tst.platformFeePaid = tst.platformFeePaid.plus(totalFee)
		tst.openFeePaid = (tst.openFeePaid ? tst.openFeePaid! : BigInt.zero()).plus(params._openFee)
		tst.closeFeePaid = (tst.closeFeePaid ? tst.closeFeePaid! : BigInt.zero()).plus(params._closeFee)
